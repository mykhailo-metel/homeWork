...
public class Bank {
    ...
    public static void withdraw(int balance, int withdrawal) {
        ...
        if (balance >= totalWithdrawal) {
            System.out.println(String.format("OK\t%.2f\t%.2f", withdrawal * COMISSION, balance - totalWithdrawal));
        } else {
     ...
     */
    public static void withdrawFromOneOfArray(int[] balances, String[] ownerNames, String ownerName, double withdrawal) {

        //Я предполагаю, что ownerNames содержит уникальные идентификаторы счета

        //Надо удостовериться, что массивы одной длины
